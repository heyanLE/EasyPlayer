...
class ExoPlayerEngine(
    ...
    private var playerEventListener: IPlayerEngine.EventListener? = null
...
//        }
    ...
    override fun clearSurfaceHolder(holder: SurfaceHolder) {
        if (::internalPlayer.isInitialized){
            internalPlayer.clearVideoSurfaceHolder(holder)
        }
    }
    ...
    override fun setEventListener(eventListener: IPlayerEngine.EventListener) {
        this.playerEventListener = eventListener
    }

    override fun removeEventListener(eventListener: IPlayerEngine.EventListener) {
    ...
    override fun onPlaybackStateChanged(playbackState: Int) {
        ...
        if (isPreparing) {
            if (playbackState == Player.STATE_READY) {
                ...
                playerEventListener?.onOtherPlayerEvent(OtherPlayerEvent.OTHER_PLAYER_EVENT_RENDERING_START)
                isPreparing = false
            }
            return
        }
        when (playbackState) {
            Player.STATE_BUFFERING -> playerEventListener?.onOtherPlayerEvent(
                OtherPlayerEvent.OTHER_PLAYER_EVENT_BUFFERING_START,
                getBufferedPercentage()
            )
            Player.STATE_READY -> playerEventListener?.onOtherPlayerEvent(
                OtherPlayerEvent.OTHER_PLAYER_EVENT_BUFFERING_END,
                getBufferedPercentage()
            )
    ...
    override fun onVideoSizeChanged(videoSize: VideoSize) {
        ...
        if (videoSize.unappliedRotationDegrees > 0) {
            playerEventListener?.onOtherPlayerEvent(
